  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6">
      <div className="mb-6 flex items-center justify-between">
        <Link to="/tuitions" className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-brand-700">
      ...
      <div className="card p-6 sm:p-8">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <span className={`badge ${tuition.status === 'closed' ? 'bg-slate-100 text-slate-500' : ''}`}>
        ...
        {tuition.description && (
          <div className="mt-6">
            ...
            <p className="mt-1 whitespace-pre-line text-sm text-slate-600">{tuition.description}</p>
          </div>
        )}
        ...
        <div className="mt-6 flex items-center gap-3 border-t border-slate-100 pt-5">
          ...
          <div className="text-sm">
            <p className="font-medium text-slate-800">Posted by {tuition.createdBy?.name}</p>
            {isOwner && tuition.createdBy?.phone && (
              <p className="flex items-center gap-1 text-slate-500"><Phone size={13} /> {tuition.createdBy.phone}</p>
            )}
          </div>
        </div>
      </div>
      ...
      {isTutor && !isOwner && tuition.status === 'open' && (
        <div className="card mt-6 p-6">
          <h3 className="font-semibold text-slate-900">Apply for this tuition</h3>
          <textarea
      ...
      {isSeeker && !isOwner && (
        <p className="mt-6 text-center text-sm text-slate-400">Only tutors can apply to tuitions.</p>
      )}
      ...
      {isOwner && (
        <div className="mt-6">
          ...
          {applications.length === 0 ? (
            <div className="card p-6 text-center text-sm text-slate-500">No applications yet.</div>
          ) : (
              ...
              {applications.map((a) => (
                <div key={a._id} className="card flex flex-wrap items-center gap-4 p-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <Link to={`/tutors/${a.tutor?._id}`} className="font-semibold text-slate-900 hover:text-brand-700">
                      {a.tutor?.name}
                    </Link>
                    <p className="text-xs text-slate-500">{a.tutor?.department}</p>
                  ...
                  {a.status === 'pending' ? (
                    ...
                    <span className={`badge ${a.status === 'rejected' ? 'bg-red-50 text-red-600' : ''}`}>
                      {cap(a.status)}
                    </span>
                  )}
                </div>
