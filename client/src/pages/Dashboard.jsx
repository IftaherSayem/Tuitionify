...
/* ── Tutor view: profile summary + my applications ─────────────── */
      ...
      <div className="card p-6">
        <div className="flex items-center gap-4">
          <img
            ...
            alt="" className="h-16 w-16 rounded-full ring-2 ring-brand-100"
          />
          <div className="flex-1">
            ...
            <p className="text-sm text-slate-500">{profile.department || 'Add your department'}</p>
            ...
            {requests.map((r) => (
              <div key={r._id} className="card flex flex-wrap items-center gap-3 p-4">
                ...
                <div className="min-w-0 flex-1">
                  <p className="font-semibold text-slate-900">{r.seeker?.name || 'A guardian'}</p>
                ...
                {r.status === 'pending' ? (
                  ...
                  <span className={`badge ${r.status === 'declined' ? 'bg-red-50 text-red-600' : ''}`}>
          ...
          <div className="space-y-3">
            {apps.map((a) => (
              <div key={a._id} className="card flex flex-wrap items-center gap-3 p-4">
                <div className="min-w-0 flex-1">
                  {a.tuition ? (
                    <Link to={`/tuitions/${a.tuition._id}`} className="font-semibold text-slate-900 hover:text-brand-700">
                      {a.tuition.title}
                    </Link>
                  ) : (
                    <span className="text-slate-400">Tuition removed</span>
                  )}
                  {a.tuition && (
...
/* ── Seeker view: my posted tuitions ───────────────────────────── */
        ...
        <div className="space-y-3">
          {tuitions.map((t) => (
            <div key={t._id} className="card flex flex-wrap items-center gap-3 p-4">
              <div className="min-w-0 flex-1">
                <Link to={`/tuitions/${t._id}`} className="font-semibold text-slate-900 hover:text-brand-700">
                  {t.title}
                </Link>
                <p className="text-xs text-slate-500">
              ...
              <span className={`badge ${t.status === 'closed' ? 'bg-slate-100 text-slate-500' : ''}`}>
