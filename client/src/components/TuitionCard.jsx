...
export default function TuitionCard({ tuition, isBookmarked, onToggleBookmark }) {
  return (
    <Link to={`/tuitions/${tuition._id}`} className="card-hover group block overflow-hidden p-5 focus:outline-none focus:ring-2 focus:ring-brand-500">
        ...
        <div className="flex shrink-0 items-center gap-2">
          {onToggleBookmark && (
            <button
              ...
              className={`icon-button -mr-2 -mt-2 ${isBookmarked ? 'text-brand-600 dark:text-brand-400' : ''}`}
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 border-t border-slate-100 pt-4 text-sm text-slate-600 dark:border-slate-700 dark:text-slate-400">
        <span className="flex items-center gap-1.5"><MapPin size={15} className="text-slate-400 dark:text-slate-500" /> {tuition.area}</span>
        <span className="flex items-center gap-1.5 font-semibold text-brand-700 dark:text-brand-300"><Wallet size={15} /> {CURRENCY}{tuition.salary?.toLocaleString()}/mo</span>
